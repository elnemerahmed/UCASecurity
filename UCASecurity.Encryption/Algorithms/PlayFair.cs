...
{
    public class PlayFair : Algorithm<string, string, string>
	{
		...
		public override Result<string> Encrypt(string text, string key)
		{
			try
			{
				string enc = Cipher(text, key, true);
				return new Result<string> { status = StatusCode.OK, payload = enc };
			}
			catch (Exception)
			{
		...
		public override Result<string> Decrypt(string cipher, string key)
		{
			try
			{
				string enc = Cipher(cipher, key, false);
				return new Result<string> { status = StatusCode.OK, payload = enc };
			}
			catch (Exception)
			{
